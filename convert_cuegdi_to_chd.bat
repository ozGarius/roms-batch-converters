...
::::: ---- PROGRAMMING ----
CALL :Startup
ECHO %_fBRed%^>^>%_fGreen% %formatIN1upper%/%formatIN2upper% to %formatOUT1upper% Converter %_RESET%
...
IF "%~x1" EQU ".%formatIN1%" || "%~x1" EQU ".%formatIN2%" (
	CALL :Compress "%~1"
) ELSE (
...
chdman createcd -i "%tmpPath%\%file%" -o "%tmpPath%\%name%.%formatOUT1%"


:: Moving the output file back to the original folder
IF EXIST "%tmpPath%\*.%formatOUT1%" (
    FOR %%x IN ("%tmpPath%\*.%formatOUT1%") DO IF NOT %%~zx==0 (
...
Set _fBRed=[91m
Set _RESET=[0m
CALL :upper %formatIN1% formatIN1upper
CALL :upper %formatIN2% formatIN2upper
CALL :upper %formatOUT1% formatOUT1upper
CALL :upper %formatOUT2% formatOUT2upper
GOTO:eof
...
:MoveBack
ECHO %_fBRed%^>^>%_fGreen% Moving %formatOUT1upper% file back to position. %_RESET%
ROBOCOPY "%tmpPath%" "%originalFolder%" "*.%formatOUT1%" /MOVE /MT:16 /COMPRESS
GOTO:eof
...
:: Help and End
:Help
ECHO Try running this on a folder with %formatIN1% or %formatIN2% files
ECHO or using one of the following syntaxes:
ECHO [script] [folder]
ECHO [script] [file.%formatIN1%]
ECHO The script will not work if there are no %formatIN1% or %formatIN2% files.
ECHO.
GOTO :End
