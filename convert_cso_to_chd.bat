...
:: Decompressing the CSO
ECHO %_fBRed%^>^>%_fGreen% Decompressing: %file% %_RESET%
CD "%tmpPath%\"
maxcso.exe --decompress "%file%" --output-path="%tmpPath%" && (
	(ECHO %_fBRed%^>^>%_fGreen% Done decompressing to "%tmpPath%" %_RESET%)
	) || (
	ECHO %_fBRed%^>^>%_fGreen% Decompressing failed. %_RESET%
	GOTO :Fail
	)
ECHO.

:: Compressing the new ISO file
ECHO %_fBRed%^>^>%_fGreen% Compressing: %name% %_RESET%
CALL chdman createcd -i "%tmpPath%/%name%.iso" -o "%tmpPath%/%name%.%formatOUT%"
ECHO.
