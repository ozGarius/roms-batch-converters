...
:: Compression level: 1-22 Default 5 in dolphintool (higher=slower but more compression)
SET compressLevel=22
...
:Folder
ECHO %_fBRed%^>^>%_fGreen% Folder given. "%~1" %_RESET%
CALL :Batch "%~1"
GOTO :End
...
:: Extracting and Compressing the ISO file
ECHO %_fBRed%^>^>%_fGreen% Extracting archive: %file% %_RESET%
ECHO %_fBRed%^>^>%_fGreen% To: "%tmpPath%" %_RESET%
7ZA x %file% -o"%tmpPath%" && (
	ECHO %_fBRed%^>^>%_fGreen% Extracting succeded, Compressing... %_RESET%
	CALL DolphinTool.exe convert --user="%tmpPath%" --input="%tmpPath%\*.iso" --output="%tmpPath%\%name%.%formatOUT%" --format=%formatOUT% --compression=zstd --block_size=131072 --compression_level=%compressLevel% 
	ECHO.
	)

:: Moving the compressed file back to the original folder and cleaning up
...
:Fail
ECHO.
ECHO %_fBRed%^>^>%_fGreen% FAILED %_RESET%
CALL :Cleanup
GOTO:eof
