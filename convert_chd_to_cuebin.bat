...
::::: ---- PROGRAMMING ----
CALL :Startup
ECHO %_fBRed%^>^>%_fGreen% %formatINupper% to %formatOUT1upper%-%formatOUT2upper% Converter %_RESET%
...
chdman verify -i "%tmpPath%\%file%" && (
	chdman extractcd -i "%tmpPath%\%file%" -o "%tmpPath%\%name%.%formatOUT1%" -ob "%tmpPath%\%name%.%formatOUT2%"
) || (
	ECHO %_fBRed%^>^>%_fGreen% "%tmpPath%\%file%" Verification failed. %_RESET%
    CALL :Fail
)
...
:MoveBack
ECHO %_fBRed%^>^>%_fGreen% Moving %formatOUT1upper%-%formatOUT2upper% file back to position. %_RESET%
ROBOCOPY "%tmpPath%" "%originalFolder%" "*.%formatOUT1%" /MOVE /MT:16 /COMPRESS
ROBOCOPY "%tmpPath%" "%originalFolder%" "*.%formatOUT2%" /MOVE /MT:16 /COMPRESS
GOTO:eof
