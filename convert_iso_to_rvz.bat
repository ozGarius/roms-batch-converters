...
:: Compression level: 1-22 Default 5 (higher=slower but more compression)
...
IF %copyLocally% == 1 (ROBOCOPY "%originalFolder%" "%tmpPath%" "%file%" /MT:16)

:: Compressing the ISO file
ECHO %_fBRed%^>^>%_fGreen% Compressing: "%name%" %_RESET%
CALL DolphinTool.exe convert --user="%tmpPath%" --input="%tmpPath%\%file%" --output="%tmpPath%\%name%.%formatOUT%" --format=%formatOUT% --compression=zstd --block_size=131072 --compression_level=%compressLevel% 
ECHO.
...
ROBOCOPY "%tmpPath%" "%originalFolder%" "*.%formatOUT%" /MOVE /MT:16
GOTO:eof
